...
namespace OpenQA.Selenium.Remote
{
    ...
    /// </summary>
    internal class HttpAuthenticatedCommandExecutor : ICommandExecutor
    {
        ...
        protected static readonly TimeSpan DefaultCommandTimeout = TimeSpan.FromSeconds(500);

        public HttpAuthenticatedCommandExecutor(Uri addressOfRemoteServer)
            : this(addressOfRemoteServer, DefaultCommandTimeout, true)
        {
        }
        ...
        public HttpAuthenticatedCommandExecutor(Uri addressOfRemoteServer, TimeSpan timeout)
        ...
        public HttpAuthenticatedCommandExecutor(Uri addressOfRemoteServer, TimeSpan timeout, bool enableKeepAlive)
        {
            ...
            user = GetUsername(addressOfRemoteServer);

            password = GetPassword(addressOfRemoteServer);
        ...
        public static string GetUsername(Uri uri)
        {
            ...
            return items.Length > 0 ? Uri.UnescapeDataString(items[0]) : string.Empty;
        }
        ...
        public static string GetPassword(Uri uri)
        {
            ...
            var items = uri.UserInfo.Split(new[] { ':' });
            return items.Length > 1 ? Uri.UnescapeDataString(items[1]) : string.Empty;
        }
        ...
        public virtual Response Execute(Command commandToExecute)
        {
            ...
            CommandInfo info = CommandInfoRepositoryExtended.Instance.GetCommandInfo(commandToExecute.Name);
